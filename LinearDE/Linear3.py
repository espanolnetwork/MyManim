%%manim -qm BernoulliMethodScene

from manim import *

class BernoulliMethodScene(Scene):
    def construct(self):
        # Color definitions
        ...
        # Title
        title = Text("Solving via Bernoulli Method (y = u * v)", font_size=32, color=BLUE)
        title.to_edge(UP)
        ...
        # ----------------------------------------------------------------
        # STEP 2: Product Substitution
        # ----------------------------------------------------------------
        step2_text = Tex("Substitute $y = u v \\Rightarrow y' = u'v + uv'$:", font_size=40)
        ...
        # ----------------------------------------------------------------
        # STEP 4: First Equation (Find v)
        # ----------------------------------------------------------------
        step4_text = Tex("Let $v' + P(x)v = 0$ to find a specific function $v(x)$:", font_size=40)
        step4_text.move_to(UP * 1.8)
        
        eq_find_v = MathTex("\\frac{dv}{v} = -P(x)dx \\implies v(x) = e^{-\\int P(x)dx}")
        eq_find_v.set_color(COLOR_HIGHLIGHT)
        eq_find_v.next_to(step4_text, DOWN, buff=0.4).scale(0.9)
        ...
        # ----------------------------------------------------------------
        # STEP 5: Second Equation (Find u)
        # ----------------------------------------------------------------
        step5_text = Tex("With grouped term as 0, solve for $u(x)$:", font_size=40)
        step5_text.move_to(UP * 1.8)

        eq_find_u = MathTex("u'v = Q(x) \\implies u(x) = \\int \\frac{Q(x)}{v(x)}dx + C")
        eq_find_u.next_to(step5_text, DOWN, buff=0.4).scale(0.9)
        ...
        # STEP 6: Conclusion
        # ----------------------------------------------------------------
        conclusion_text = Tex("Multiply $u(x)$ and $v(x)$ to get the final answer:", font_size=40)
        conclusion_text.set_color_by_tex("final answer", COLOR_RESULT)
        conclusion_text.move_to(DOWN * 0.8)
        
        eq_final = MathTex("y = u(x) \\cdot v(x)")
